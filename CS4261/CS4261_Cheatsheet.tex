...
The GS Algorithm (with student proposing) assigns each student $s \in S$ to the hospital $\text{best}(s)$, and each hospital $h \in H$ to the student $\text{worst}(h)$.
...
\\
Valuation is \deff{Additive} if 
\[
v_i(G') = \sum_{g\in G'} v_i(g), \ \forall G' \subseteq G 
\]
