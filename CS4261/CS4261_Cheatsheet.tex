...
\subsection{Definitions}
\subsubsection{Problem Specifications}
...
\begin{itemize}
    \item \deff{Social Welfare} - $\sum_{i \in N} u_i(W)$, total number of approvals committee members receive
    \item \deff{Coverage} - Total number of voters who approves at least one committee member.
\end{itemize}
A candidate \deff{Covers} a voter if the voter approves the candidate.
\\
Some solutions based on the two
\begin{itemize}
    \item \deff{Approval Voting} (AC) - Select a committee that maximises social welfare,
    \[
    W_{AC} = \max_{W \subseteq C, |W| = k} \sum_{i \in N} u_i(W)
    \]
    \item \deff{Chamberlin-Courant} (CC) - Select a committee that maximises coverage,
    \[
    W_{CC} = \max_{W \subseteq C, |W| = k} |\{ i \in N | A_i \cap W \neq \emptyset \}|
    \]
\end{itemize}
\underline{Note}: Both solutions are not unique.
