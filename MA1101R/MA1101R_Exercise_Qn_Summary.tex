...
\subsection{Ex4Qn22}
Let $A$ be an $m \times n$ matrix and $P$ an $m \times m$ matrix. If $P$ is invertible, then $rank(PA) = rank(A)$.
...
\begin{itemize}
    ...
    \item $nullity(A) = nullity(A^T A)$
    \item $rank(A) = rank(A^T A)$
    \item $rank(A) = rank(A A^T)$
\end{itemize}
(However, $nullity(A) \neq nullity(AA^T)$)
...
\subsection{Ex7Qn16}
Let $T : \mathbb{R^n} \to \mathbb{R^n}$ be a linear transformation. $Ker(T) = \{ \pmb 0 \}$ iff T is one-to-one (i.e. $\forall \ \pmb u, \pmb v \in \mathbb{R^n}$, $\pmb u \ne \pmb v \to T(\pmb u) \ne T(\pmb v)$)
