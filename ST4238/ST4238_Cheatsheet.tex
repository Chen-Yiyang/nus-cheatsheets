...
To \textbf{homogeniz a non-homogeneous Poisson Process}:
\begin{enumerate}
    \item Define $\Lambda (t) = \int_0 ^t \lambda(u) du$
    \item Define a new process $Y(s) = X(t)$ where $s = \Lambda(t)$
    \item Now $Y(s)$ is a homogeneous Poisson Process with rate $1$.
\end{enumerate}
...
\\
\underline{Note}: For an absorbing state $i$ (e.g. $i = 0$ in Birth \& Death Process), we may set $\nu_i = 0$.
