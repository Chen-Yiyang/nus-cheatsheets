...
\subsubsection{Variable Order Heuristics}
...
\subsubsection{Value-Order Heuristic}
...
\deff{Backtracing Algorithm} uses DFS to examine value assignments of one variable at a time (one level), and backtrack when no possible legal assignments with current assignment of the variable. Heuristics for value / variable choices and pruning techniques can be used.
...
A game model consists of 1) \deff{State representation}, 2) \codein{TO-MOVE(s)}, a function that returns the player to move for state $s$, 3) \codein{RESULT(s, a)}, the transition model that returns resultant state after taking action $a$ at state $s$, 3) \codein{IS-TERMINAL(s)}, whether the game has ended at state $s$, and 4）\codein{UTILITY(s, p)} that defines the final numeric value to $p$ when the game ends in terminal state $s$.
\\
...
\deff{MiniMax Optimal Strategy} - MAC chooses the move that maximise the minimum payoff while MIN chooses to minimise the maximum payoff.
\\
...
A \deff{Knowledge-Based Agent} is an agent that makes inferences on existing information to infer new information. It consists of an \deff{Inference Engine} and a \deff{Knowledge Base}, which is a set of sentences (logical formulae).
...
A valid sentence is a tautology. Proof by contradiction uses the idea that something is true == its negation is unsatisfiable.

% TODO: if need inference algo.
...
\underline{Procedure}: 1) Make a clause list (KB in CNF, with \textbf{Negation of Query $\alpha$}). 2) Repeatedly resolve two clauses and add resolvent to list. 3) Stop when empty clause $\emptyset$ found, which implies $\alpha$ CAN be inferred, or when no more resolutions possible, which implies $\alpha$ CANNOT be inferred. \textbf{Note}: Cannot be inferred $\neq$ Is incorrect / negation can be inferred.
\\
...
Sources of Uncertainty include: 1) Partial observability, 2) Noisy Sensors, 3) Uncertainty in action outcomes, or 4) Complexity in modelling and predicting the problem.

\smallskip

In handling uncertainty, a general logical agent will 1) Risk Falsehood, and/or 2) Reach weaker conclusions.
